/**************************************************************************************************
 *
 * Filename           : main_3.3.5_VerifySignedData.c
 * Author             : Utimaco GmbH
...
int main(int argc, char *argv[])
{
  ...
  char                  *Data    = "The standard PKCS11 specifies an application programming interface (API), \n" \
  ...
  err = SignData(pFunctions,hSession,hPrivateKey,(CK_BYTE_PTR) Data,lenData,&signature,&lenSignature);
  if (err != 0) goto cleanup;

  // verify signed data - PASS CASE
  printf("\n-> C_Verify() PASS CASE, handed over text used for signing\n");
  err = VerifySignedData(pFunctions,hSession,hPublicKey,(CK_BYTE_PTR) Data,lenData,signature,lenSignature);
  if (err != 0) goto cleanup;

  // verify signed data - FAIL CASE
  printf("\n-> C_Verify() FAIL CASE, handed over text different from that used for signing\n");
  err = VerifySignedData(pFunctions,hSession,hPublicKey,(CK_BYTE_PTR)"FAIL CASE",(CK_ULONG)strlen("FAIL CASE"),signature,lenSignature);
  if (err != 192) goto cleanup;
